...
import {
  ArticleList,
  ArticleListSkeleton,
} from "@/components/news/article-list";
...
export default function News() {
  ...
  const parsedQuery = useMemo(
    () => qs.parse(router.query, { comma: true }),
    [router.query],
  );
  ...
  const getFullTagsFromIds = useCallback(
    (type, tags) => {
      ...
      if (Array.isArray(tags) && !tags.every((tag) => typeof tag === "string"))
        throw new Error(
          "`tags` is an array and some elements of the array aren't strings",
        );
  ...
  useEffect(() => {
    if (allTagsLoading) return;
    _setSelectedTags({
      collaborations: getFullTagsFromIds(
        "collaborations",
        parsedQuery.collaborations,
      ),
      people: getFullTagsFromIds("people", parsedQuery.people),
      projects: getFullTagsFromIds("projects", parsedQuery.projects),
      organizations: getFullTagsFromIds(
        "organizations",
        parsedQuery.organizations,
      ),
      researchGroups: getFullTagsFromIds(
        "researchGroups",
        parsedQuery.researchGroups,
      ),
      ...
      freeSearch:
        parsedQuery.freeSearch === undefined
          ...
          : typeof parsedQuery.freeSearch === "string"
            ...
            : Array.isArray(parsedQuery.freeSearch) &&
                parsedQuery.freeSearch.every((s) => typeof s === "string")
              ? parsedQuery.freeSearch
              : [],
    });
  ...
  const isTagSelected = useCallback(
    (id, type) => {
      ...
      return (
        selectedTags[type]?.find(
          (tag) =>
            tag[
              type === "freeSearch" || type === "postTags" ? "name" : "slug"
            ] === id,
        ) !== undefined
      );
    },
  ...
  const setFlatSelectedTags = useCallback(
    (value) => {
      ...
      const next =
        typeof value === "function" ? value(flatSelectedTags) : value;
      ...
      const queryParamObj = {
        ...(parsedQuery.blogOrFeature && {
          blogOrFeature: parsedQuery.blogOrFeature,
        }),
        ...(parsedQuery.page && { page: parsedQuery.page }),
        ...(next.some((t) => t.type === "researchGroups") && {
          researchGroups: next
            .filter((t) => t.type === "researchGroups")
            .map((t) => t.slug),
        }),
        ...(next.some((t) => t.type === "collaborations") && {
          collaborations: next
            .filter((t) => t.type === "collaborations")
            .map((t) => t.slug),
        }),
        ...(next.some((t) => t.type === "projects") && {
          projects: next
            .filter((t) => t.type === "projects")
            .map((t) => t.slug),
        }),
        ...(next.some((t) => t.type === "organizations") && {
          organizations: next
            .filter((t) => t.type === "organizations")
            .map((t) => t.slug),
        }),
        ...
        ...(next.some((t) => t.type === "postTags") && {
          postTags: next
            .filter((t) => t.type === "postTags")
            .map((t) => t.name), // name instead of slug
        }),
        ...(next.some((t) => typeof t === "string") && {
          // freeSearch items are just strings rather than objs
      ...
      router.push(
        {
          pathname: "/news",
          query: qs.stringify(queryParamObj, {
            arrayFormat: "comma",
  ...
  const deleteTag = useCallback(
    (id, type) => {
      const tagIndexToDelete = flatSelectedTags.findIndex((tag) => {
        if (typeof tag === "string" && type === "freeSearch" && id === tag)
          return true;
      ...
      setFlatSelectedTags(
        deleteIndexFromArray(flatSelectedTags, tagIndexToDelete),
      );
    },
  ...
  const addTag = useCallback(
    (id, type) => {
      ...
      const fullTag = allTags[type].find(
        (tag) => tag[type === "postTags" ? "name" : "slug"] === id,
      );
  ...
  const setBlogOrFeature = useCallback(
    (value) => {
  ...
  return (
    <Page
      title="News"
      description="View and sort through RENCI news articles and blog posts."
    >
      <Typography>
        Use the search box below to filter the list of articles by tag or
        category. If there isn’t a pre-existing item, you can type your query
        and press enter to search through the article titles. Use the “filters”
        sidebar to verify and refine your search. If you would like to view
        articles about RENCI from other publications, visit the{" "}
        <Link href="/news/appearances">news appearances page</Link>.
      </Typography>
...
const FilterSelectorContents = ({
  ...
  <>
    ...
    >
      <Typography variant="h3" maxWidth="fit-content">
        Filters
      </Typography>
