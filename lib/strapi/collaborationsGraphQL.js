...
export const fetchStrapiCollaboration = async (id, preview = false) => {
  const { data: collaborationData } = await fetchStrapiGraphQL(
    `query {
  ...
  const memberPIDs =
    await collaborationData.collaborations.data[0].attributes.members.data.map(
  ...
  const payload = {
    ...
    description:
      collaborationData.collaborations.data[0].attributes.description,
    featuredImage:
      collaborationData.collaborations.data[0].attributes?.featuredImage?.data
        ?.attributes,
    ...
    projects: collaborationData.collaborations.data[0].attributes.projects.data
      ? collaborationData.collaborations.data[0].attributes.projects.data.map(
          (project) => ({
      ...
      : [],
    partners: collaborationData.collaborations.data[0].attributes.partners.data
      ? collaborationData.collaborations.data[0].attributes.partners.data.map(
          (partner) => ({
      ...
      : [],
    funding: collaborationData.collaborations.data[0].attributes.funding.data
      ? collaborationData.collaborations.data[0].attributes.funding.data.map(
          (partner) => ({
            slug: partner.attributes.slug,
            name: partner.attributes.name,
            orgURL: partner.attributes.url,
