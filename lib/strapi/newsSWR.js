  ...
  const fetcher = (url) =>
    fetch(url, {
      ...
      .then((data) => {
        if (data.error)
          throw new Error(
            `Strapi Error: ${data.error?.message ?? "Unknown error while fetching post tags"}`,
          );
        return data;
      });

  // dedupingInterval essentially sets cache expiration time to 1 hour
  return useSWR(getStrapiURL("/api/all-post-tags"), fetcher, {
  ...
  const fetcher = (cacheKey) => {
    ...
    return fetch(articlesUrl, {
      ...
      .then((data) => {
        if (data.error)
          throw new Error(
            `Strapi Error: ${data.error?.message ?? "Unknown error while fetching articles"}`,
          );
        return data;
      });
  };
