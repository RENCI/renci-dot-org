...
export const fetchPhotoThumbnailUrl = async (slug) => {
  ...
  `);
  return (
    res?.data?.people?.data?.[0]?.attributes?.photo?.data?.[0]?.attributes
      ?.formats?.thumbnail ?? null
  );
};
...
export const fetchArticle = async (slug) => {
  ...
  let photos = await Promise.allSettled(
    articleGql.data.posts.data[0].attributes.renciAuthors.data.map(
      ({ attributes }) => fetchPhotoThumbnailUrl(attributes.slug),
    ),
  );
  ...
  return await articleGql.data.posts.data.map(({ attributes }) => ({
    ...
    researchGroups: attributes.researchGroups.data.map(
      ({ attributes }) => attributes,
    ),
    collaborations: attributes.collaborations.data.map(
      ({ attributes }) => attributes,
    ),
    projects: attributes.projects.data.map(({ attributes }) => attributes),
    organizations: attributes.organizations.data.map(
      ({ attributes }) => attributes,
    ),
