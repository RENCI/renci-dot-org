...
export function FlexSpaceCarousel({ slides, onCurrentSlideIndexChange }) {
  ...
  const getIndexCompletelyInView = useCallback((e) => {
    const indicies = e.slidesInView();
    onCurrentSlideIndexChange(indicies[0]);
  }, [onCurrentSlideIndexChange]);

  useEffect(() => {
    if (emblaApi) {
      emblaApi.on('slidesInView', getIndexCompletelyInView)
    }
    return () => { 
      emblaApi?.off('slidesInView', getIndexCompletelyInView)
    }
  }, [emblaApi, getIndexCompletelyInView]);
